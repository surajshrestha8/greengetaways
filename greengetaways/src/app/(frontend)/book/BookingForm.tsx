...
export default function BookingForm({
  ...
  const usingCustomDate = formData.isCustomDate || !hasDepartureDates
  const selectedDeparture = usingCustomDate
    ? null
    : departureDates.find(
  ...
  const validate = (): boolean => {
    ...
    if (usingCustomDate) {
      const normalized = normalizeDate(formData.departureDate)
      if (!normalized) {
        newErrors.departureDate = 'Please select a preferred date'
  ...
  const handleChange = (
    ...
    if (name === 'customDate') {
      setFormData((prev) => ({ ...prev, departureDate: value, departureId: '' }))
  ...
  const toggleCustomDate = () => {
    setFormData((prev) => ({
      ...prev,
      isCustomDate: !prev.isCustomDate,
      departureDate: '',
      departureId: '',
    }))
    setErrors((prev) => ({ ...prev, departureDate: undefined, numberOfTravelers: undefined }))
    ...
    try {
      const res = await fetch('/api/book', {
        ...
        body: JSON.stringify({
          ...
          departureId: usingCustomDate ? undefined : formData.departureId,
          isCustomDate: usingCustomDate,
      ...
      <form className="booking-form-card" onSubmit={handleSubmit} noValidate>
        ...
        <div className="form-section">
          ...
          <div className="form-group">
            ...
            {hasDepartureDates && !formData.isCustomDate ? (
              <>
                <select
                  id="departureDate"
                  name="departureId"
              ...
              <>
                ...
                />
                <p className="form-help-text">
                  Choose your preferred travel date. Our team will confirm availability and get back
                  to you.
                </p>
              </>
            )}
            ...
            {hasDepartureDates && (
              <button type="button" className="custom-date-toggle" onClick={toggleCustomDate}>
                {formData.isCustomDate ? (
                  <>
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                      <polyline points="15 18 9 12 15 6" />
                    </svg>
                    Back to scheduled dates
                  </>
                ) : (
                  <>
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
        ...
        <button type="submit" className="submit-btn" disabled={isSubmitting || (!hasBookableDeparture && !usingCustomDate)}>
          {isSubmitting ? (
            <>
